...
> Implementation conventions that protect lifecycle correctness, transcript accuracy, Gemma sanitization, and docs/code alignment.
...
- If model output contains reasoning traces or object serialization noise, sanitize it before showing or speaking it.
...
- Gemma routes should return friendly text for user-facing failures instead of raw provider blobs.
...
- Keep Agora branding and Gemma branding visible but restrained; do not crowd the header with duplicate marks.
...
- For route or contract changes, run `pnpm run lint` and `pnpm run typecheck` first.
- For UI/client runtime changes, run at least lint + typecheck + build.
- Prefer narrow checks during iteration, full local smoke tests before handoff.

## Pull Request Hygiene

- Keep scope small and copyable for demo consumers.
